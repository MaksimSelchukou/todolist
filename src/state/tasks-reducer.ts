...
type actionsType = addTaskType | removeTaskType | changeStatusTaskType | changeTitleTaskType;
...
export const tasksReducer = (state: TaskStateType, action: actionsType): TaskStateType => {
    switch (action.type) {
        ...
        case "CHANGE-STATUS-TASK":{
            return {...state,[action.todoID]:state[action.todoID].map(m=>m.id === action.taskID ?{...m,isDone:action.isDone}:m)}
        }
